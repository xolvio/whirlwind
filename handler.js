...
const impl = {
  ...
  getSettings: script => {
    const ret = {
      maxScriptDurationInSeconds:
        constants.DEFAULT_MAX_SCRIPT_DURATION_IN_SECONDS,
      maxScriptRequestsPerSecond:
        constants.DEFAULT_MAX_SCRIPT_REQUESTS_PER_SECOND,
      maxChunkDurationInSeconds:
        constants.DEFAULT_MAX_CHUNK_DURATION_IN_SECONDS,
      maxChunkRequestsPerSecond:
        constants.DEFAULT_MAX_CHUNK_REQUESTS_PER_SECOND,
      timeBufferInMilliseconds:
        constants.DEFAULT_MAX_TIME_BUFFER_IN_MILLISECONDS
    };
    if (script._split) {
      if (script._split.maxScriptDurationInSeconds) {
        ret.maxScriptDurationInSeconds =
          script._split.maxScriptDurationInSeconds;
      }
      ...
      if (script._split.maxScriptRequestsPerSecond) {
        ret.maxScriptRequestsPerSecond =
          script._split.maxScriptRequestsPerSecond;
      }
  ...
  scriptDurationInSeconds: script => {
    ...
    for (i = 0; i < script.config.phases.length; i++) {
      phaseDurationInSeconds = impl.phaseDurationInSeconds(
        script.config.phases[i]
      );
  ...
  scriptRequestsPerSecond: script => {
    ...
    for (i = 0; i < script.config.phases.length; i++) {
      phaseRequestsPerSecond = impl.phaseRequestsPerSecond(
        script.config.phases[i]
      );
  ...
  validScript: (script, context, callback) => {
    ...
    if (script._split && typeof script._split !== "object") {
      ...
      !(
        ...
        settings.maxChunkDurationInSeconds <=
          constants.MAX_CHUNK_DURATION_IN_SECONDS
      )
    ) {
      callback(
        'If specified the "_split.maxChunkDurationInSeconds" attribute must be an integer inclusively between ' +
      ...
      !(
        ...
        settings.maxScriptDurationInSeconds <=
          constants.MAX_SCRIPT_DURATION_IN_SECONDS
      )
    ) {
      callback(
        'If specified the "_split.maxScriptDurationInSeconds" attribute must be an integer inclusively between ' +
      ...
      !(
        ...
        settings.maxChunkRequestsPerSecond <=
          constants.MAX_CHUNK_REQUESTS_PER_SECOND
      )
    ) {
      callback(
        'If specified the "_split.maxChunkRequestsPerSecond" attribute must be an integer inclusively ' +
      ...
      !(
        ...
        settings.maxScriptRequestsPerSecond <=
          constants.MAX_SCRIPT_REQUESTS_PER_SECOND
      )
    ) {
      callback(
        'If specified the "_split.maxScriptRequestsPerSecond" attribute must be an integer inclusively ' +
      ...
      !(
        ...
        settings.timeBufferInMilliseconds <=
          constants.MAX_TIME_BUFFER_IN_MILLISECONDS
      )
    ) {
      callback(
        'If specified the "_split.timeBufferInMilliseconds" attribute must be an integer inclusively ' +
      ...
      !(
        script.config &&
        Array.isArray(script.config.phases) &&
        script.config.phases.length > 0
      ) && // must have phase
      !(
        script.mode === constants.modes.ACC ||
        script.mode === constants.modes.ACCEPTANCE
      ) // unless in acceptance mode
    ) {
      callback(
      ...
      callback(
        `If specified, the mode attribute must be one of "${Object.keys(
          constants.modes
        )
      ...
      if (scriptDurationInSeconds < 0) {
        callback(
          `Every phase must have a valid duration in seconds.  Observed: ${JSON.stringify(
            script.config.phases[scriptDurationInSeconds * -1]
          )}`
        );
      } else if (
        scriptDurationInSeconds > settings.maxScriptDurationInSeconds
      ) {
        callback(
        ...
        callback(
          `Every phase must have a valid means to determine requests per second.  Observed: ${JSON.stringify(
            script.config.phases[scriptRequestsPerSecond * -1]
          )}`
        );
      } else if (
        scriptRequestsPerSecond > settings.maxScriptRequestsPerSecond
      ) {
        callback(
  ...
  splitPhaseByDurationInSeconds: (phase, chunkSize) => {
    ...
    if ("duration" in phase) {
      ...
      } else if ("arrivalCount" in phase) {
        // split the arrival count proportionally
        ratio = chunkSize / phase.duration;
  ...
  splitScriptByDurationInSeconds: (script, chunkSize) => {
    ...
    while (
      ...
      ret.remainder.config.phases.length
    ) {
      phase = ret.remainder.config.phases.shift();
      phaseDurationInSeconds = impl.phaseDurationInSeconds(phase);
      if (phaseDurationInSeconds > remainingDurationInSeconds) {
        // split phase
        phaseParts = impl.splitPhaseByDurationInSeconds(
          phase,
  ...
  intersect: (l1, l2) => {
    ...
    if (det === 0) {
      throw new Error(
        "Parallel lines never intersect, detect and avoid this case"
      );
    } else {
  ...
  intersection: (phase, chunkSize) => {
    const ramp = impl.abc(
      { x: 0, y: phase.arrivalRate },
      { x: phase.duration, y: phase.rampTo }
    );
    const limit = impl.abc(
      { x: 0, y: chunkSize },
  ...
  splitPhaseByRequestsPerSecond: (phase, chunkSize) => {
    ...
    if (
      "rampTo" in phase &&
      "arrivalRate" in phase &&
      phase.rampTo === phase.arrivalRate
    ) {
    ...
    if ("rampTo" in phase && "arrivalRate" in phase) {
      ...
      if (max <= chunkSize) {
        ...
        impl.addRamp(
          ret.remainder,
          phase,
          phase.arrivalRate - chunkSize,
          phase.rampTo - chunkSize,
          phase.duration
        );
      } else {
      ...
      // constant rate phase
      if (phase.arrivalRate > chunkSize) {
        // subtract the chunkSize if greater than that
        impl.addArrivalRate(ret.chunk, phase, chunkSize, phase.duration);
        impl.addArrivalRate(
          ret.remainder,
          phase,
        ...
        // Otherwise, include the entire arrival and create a pause for the remainder
        impl.addArrivalRate(
          ret.chunk,
          phase,
          phase.arrivalRate,
      ...
      if (rps >= chunkSize) {
        ...
        impl.addArrivalCount(
          ret.remainder,
          phase,
          phase.arrivalCount - arrivalCount,
          phase.duration
        );
      } else {
        impl.addArrivalCount(
  ...
  splitScriptByRequestsPerSecond: (script, chunkSize) => {
    ...
    for (i = 0; i < script.config.phases.length; i++) {
      phaseParts = impl.splitPhaseByRequestsPerSecond(
        script.config.phases[i],
        chunkSize
      );
  ...
  splitScriptByFlow: script => {
    ...
    for (i = 0; i < oldScript.scenarios.length; i++) {
  ...
  invokeSelf(timeDelay, event, context, callback, invocationType) {
    const exec = () => {
      try {
        ...
        const msg =
          `ERROR exception encountered while invoking self from ${
            event._genesis
    ...
    if (timeDelay > 0) {
      ...
      if (event._trace) {
        console.log(
          ...
          `scheduling self invocation for ${event._genesis} in ${
            event._start
          } with a ${timeDelay} ms delay`
        );
      }
    } else {
      exec();
  ...
  readPayload(script) {
    let ret;
    const determinePayloadPath = payloadFile =>
    ...
    if (script && script.config && script.config.payload) {
      ...
      if (Array.isArray(script.config.payload)) {
        ...
        console.log(
          "WARNING: payload file not set, but payload is configured.\n"
        );
      }
    }
    return ret;
  ...
  runLoad: (start, script, context, callback) => {
    ...
    if (script._simulation) {
      console.log(
        `SIMULATION: runLoad called with ${JSON.stringify(script, null, 2)}`
      );
      ...
      try {
        ...
        runner.on("phaseStarted", opts => {
          console.log(
            `phase ${opts.index}${
              opts.name ? ` (${opts.name})` : ""
            } started, duration: ${opts.duration ? opts.duration : opts.pause}` // eslint-disable-line comma-dangle
          );
        });
        ...
        msg =
          `ERROR exception encountered while executing load from ${
            script._genesis
          } ` + `in ${start}: ${ex.message}\n${ex.stack}`;
        console.log(msg);
        callback(msg);
  ...
  runPerformance: (timeNow, script, context, callback) => {
    ...
    const complete = () => {
      toComplete -= 1;
      if (toComplete > 0) {
        // do we have more outstanding asynchronous operations/callbacks to execute?
        if (script._trace) {
          console.log(
            `load test from ${
              script._genesis
        ...
        // otherwise, time to complete the lambda
        callback(null, {
          message: `load test from ${
            script._genesis
          } successfully completed from ${timeNow} @ ${Date.now()}`
        });
        if (script._trace) {
          console.log(
            `load test from ${
    ...
    if (scriptDurationInSeconds > settings.maxChunkDurationInSeconds) {
      if (script._trace) {
        console.log(
          `splitting script by duration from ${
            script._genesis
          } in ${timeNow} @ ${Date.now()}`
        );
      }
      toComplete = 2;
      parts = impl.splitScriptByDurationInSeconds(
        script,
        settings.maxChunkDurationInSeconds
      );
      ...
      parts.remainder._start =
        parts.chunk._start + settings.maxChunkDurationInSeconds * 1000;
      scriptRequestsPerSecond = impl.scriptRequestsPerSecond(parts.chunk);
      if (script._trace) {
        console.log(
          `scheduling shortened chunk start from ${
            script._genesis
          } in ${timeNow} for execution @ ${parts.chunk._start}`
        );
      }
      if (scriptRequestsPerSecond > settings.maxChunkRequestsPerSecond) {
        // needs splitting by requestsPerSecond too
        impl.runPerformance(timeNow, parts.chunk, context, complete);
      } else {
        // otherwise, send to a separate lambda for execution
        impl.invokeSelf(
          // eslint-disable-next-line comma-dangle
          parts.chunk._start - Date.now() - settings.timeBufferInMilliseconds,
          parts.chunk,
          context,
          complete
        );
      ...
      if (script._trace) {
        console.log(
          // eslint-disable-next-line comma-dangle
          `scheduling remainder start from ${
            script._genesis
          } in ${timeNow} for execution @ ${parts.remainder._start}`
        );
      }
      impl.invokeSelf(
        // eslint-disable-next-line comma-dangle
        parts.remainder._start - Date.now() - settings.timeBufferInMilliseconds,
        parts.remainder,
        context,
        complete
      );
      ...
      if (script._trace) {
        console.log(
          `splitting script by requests per second from ${
            script._genesis
          } in ${timeNow} @ ${Date.now()}`
        );
      }
      ...
      toComplete = Math.ceil(
        scriptRequestsPerSecond / settings.maxChunkRequestsPerSecond
      );
      do {
        parts = impl.splitScriptByRequestsPerSecond(
          script,
          settings.maxChunkRequestsPerSecond
        );
      ...
      if (timeDelay > 0) {
        ...
        if (script._trace) {
          console.log(
            `scheduling load execution from ${
              script._genesis
            } in ${timeNow} with a ${timeDelay} ms delay @ ${Date.now()}`
          );
        }
      } else {
        exec();
    ...
    const complete = (err, res) => {
      ...
      if (res.Payload) {
        try {
          ...
          console.log(
            `Error parsing lambda execution payload: "${res.Payload}"`
          );
        }
      }
      if (toComplete > 0) {
