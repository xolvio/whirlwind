...
module.exports.generateStressPhases = function (startLoad = 10, endLoad = 1000, rampUpTimePerStep = 20, flatDurationPerStep = 40, numberOfSteps = 6, rampUpType = 'linear') {
  ...
  for (let phase = 0; phase < numberOfSteps; phase += 1) {
    ...
    if (rampUpType === 'linear') {
      startArrivalRateForThisPhase = ((totalRampUp / numberOfSteps) * phase) + startLoad
      rampToForThisPhase = (((endLoad - startLoad) / numberOfSteps) * (phase + 1)) + startLoad
    } else if (rampUpType === 'exponential') {
      startArrivalRateForThisPhase = Math.round(startLoad + (totalRampUp * exponentialFunction(phase, numberOfSteps)))
      rampToForThisPhase = Math.round(startLoad + (totalRampUp * exponentialFunction(phase + 1, numberOfSteps)))
    }
...
module.exports.generateLoadPhases = function (startLoad = 10, endLoad = 1000, rampUpTime = 5 * 60) {
  return [
    warmUpPhase,
    {
      duration: rampUpTime,
...
module.exports.generateSpikePhases = function (consistentLoad = 30, spikeLoad = 300, loadTestDuration = 400, spikeDuration = 5, numberOfSpikes = 4) {
